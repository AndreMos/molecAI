...
class CustomDataset(QM9):
    ...
    def deg(self, data_index, positions):
        ...
        for source, (leaf1, leaf2) in enumerate(data_index[0].reshape(-1, 2)):
            ...
            angles.append(
                torch.dot(leaf1_sour, leaf2_sour)
                / (torch.norm(leaf1_sour) * torch.norm(leaf2_sour))
            )
        return angles
    ...
    def process(self):
        ...
        if True:
            ...
            atom_charges = [
                x["z"].apply_(lambda f: self.corr_dict[f]) for x in data_list
            ]
            ...
            for x in data_list:
                complete = radius_graph(
                    x["pos"], r=10, batch=None, max_num_neighbors=32
                )
                adj_mat_complete = to_dense_adj(complete).type(torch.int8)
                adj_mat_bonds = to_dense_adj(
                    x["edge_index"], edge_attr=torch.argmax(x["edge_attr"], dim=1) + 2
                )

                decreased, bond_feat = dense_to_sparse(
                    torch.tril(adj_mat_bonds | adj_mat_complete)
                )  # torch.stack([x[0] for x in sorted([(x,y) for x,y in zip(complete.T, summed)], key = lambda x:( x[1], x[0][1]))], dim=0)[::2].T
            ...
            for x, y, dist, row, col, angle, bond in zip(
                data_list,
                ...
                torch.nn.utils.rnn.pad_sequence(
                    angles, batch_first=True, padding_value=10
                ),
